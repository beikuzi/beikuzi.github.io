...
export function extractFirstImage(mdContent, title) {
  ...
  // 处理相对路径 - md文件在md子目录，图片路径是../images/
  if (imgPath.startsWith('../images/')) {
    imgPath = imgPath.substring(3); // 去掉 ../，变成 images/xxx
...
export async function loadArticleFirstImage(title) {
  try {
    // md文件在md子目录
    const res = await fetch(`/assets/articles/md/${encodeURIComponent(title)}.md`, { cache: 'no-store' });
